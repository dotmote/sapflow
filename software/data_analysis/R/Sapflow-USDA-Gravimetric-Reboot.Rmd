...
## Verify the input files of heat ratio - May 2022


```{r pressure, echo=FALSE}
sapflow_usda_2022_hr_19<-  read_csv('./data/usda/Raw Data for Sending to Dotmote_heat_ratios/sapflow-usda-19.csv')
# Tell R studio that timestamp is already in MDT
sapflow_usda_2022_hr_19$dateP <- as_datetime(sapflow_usda_2022_hr_19$date, tz="America/Denver")


sapflow_usda_2022_hr_25<-  read_csv('./data/usda/Raw Data for Sending to Dotmote_heat_ratios/sapflow-usda-25.csv')
# Tell R studio that timestamp is already in MDT
sapflow_usda_2022_hr_25$dateP <- as_datetime(sapflow_usda_2022_hr_25$date, tz="America/Denver")


sapflow_usda_2022_hr_40<-  read_csv('./data/usda/Raw Data for Sending to Dotmote_heat_ratios/sapflow-usda-40.csv')
# Tell R studio that timestamp is already in MDT
sapflow_usda_2022_hr_40$dateP <- as_datetime(sapflow_usda_2022_hr_40$date, tz="America/Denver")


sapflow_usda_2022_hr_43<-  read_csv('./data/usda/Raw Data for Sending to Dotmote_heat_ratios/sapflow-usda-43.csv')
# Tell R studio that timestamp is already in MDT
sapflow_usda_2022_hr_43$dateP <- as_datetime(sapflow_usda_2022_hr_43$date, tz="America/Denver")
...
```{r }
sapflow_usda_hr_may_june_2022 <-  rbind(sapflow_usda_2022_hr_19,
                            sapflow_usda_2022_hr_25,
                            sapflow_usda_2022_hr_40,
                            sapflow_usda_2022_hr_43)
...
```{r }
sapflow_usda_hr_may_june_2022 %>% mutate(meanhr = meanHeatRatio) %>%
  filter(!meanhr =='NaN') %>% mutate(mr=as.numeric(meanhr)) %>%
     mutate(node = as.factor(nodeID),Vh = log(mr),hr = hour(dateP),yr = year(dateP),dy = day(dateP)) %>%
      filter(date(dateP) > "2022-05-22" & date(dateP) < "2022-05-24" ) %>%
  ...
  facet_grid(dy~node) +
  theme_classic() + labs(x="Date",y="Mean HR", title = "May-June 2022 - Node 19,25 and 43 USDA - Gravimetric Maize", caption="Source : dotmotelabs.com") +
...
```{r , echo=TRUE}
colnames(sapflow_usda_hr_may_june_2022) <- c("date", "nodeId","meanHeatRatio" , "dateP"          )
...
usda_19_offset <- sapflow_usda_hr_may_june_2022 %>%  mutate(meanhr = meanHeatRatio,hr = hour(dateP)) %>%
  filter(!meanhr =='NaN') %>% 
  filter(nodeId %in% c("sapflow-usda-19"))  %>% 
  filter( hr %in% c(0,1,2)) %>% summarise(mhr = mean(meanhr))


usda_25_offset <- sapflow_usda_hr_may_june_2022 %>%  mutate(meanhr = meanHeatRatio,hr = hour(dateP)) %>%
  filter(!meanhr =='NaN') %>% 
  filter(nodeId %in% c("sapflow-usda-25"))  %>%
  filter( hr %in% c(0,1,2)) %>% summarise(mhr = mean(meanhr))

usda_40_offset <- sapflow_usda_hr_may_june_2022 %>%  mutate(meanhr = meanHeatRatio,hr = hour(dateP)) %>%
  filter(!meanhr =='NaN') %>% 
  filter(nodeId %in% c("sapflow-usda-40"))  %>% 
  filter( hr %in% c(0,1,2)) %>% summarise(mhr = mean(meanhr))

usda_43_offset <- sapflow_usda_hr_may_june_2022 %>%  mutate(meanhr = meanHeatRatio,hr = hour(dateP)) %>%
  filter(!meanhr =='NaN') %>% 
  filter(nodeId %in% c("sapflow-usda-43"))  %>% 
  filter( hr %in% c(0,1,2)) %>% summarise(mhr = mean(meanhr))


sapflow_usda_2022_adjusted <- sapflow_usda_hr_may_june_2022
sapflow_usda_2022_adjusted$hr = hour(sapflow_usda_hr_may_june_2022$dateP)
sapflow_usda_2022_adjusted$mr <- 0

sapflow_usda_2022_adjusted[sapflow_usda_2022_adjusted$nodeId == "sapflow-usda-19",]$mr =  sapflow_usda_2022_adjusted[sapflow_usda_hr_may_june_2022$nodeId == "sapflow-usda-19",]$meanHeatRatio + (1-as.numeric(usda_19_offset))

sapflow_usda_2022_adjusted[sapflow_usda_2022_adjusted$nodeId == "sapflow-usda-25",]$mr =  sapflow_usda_2022_adjusted[sapflow_usda_hr_may_june_2022$nodeId == "sapflow-usda-25",]$meanHeatRatio + (1-as.numeric(usda_25_offset))

sapflow_usda_2022_adjusted[sapflow_usda_2022_adjusted$nodeId == "sapflow-usda-40",]$mr =  sapflow_usda_2022_adjusted[sapflow_usda_hr_may_june_2022$nodeId == "sapflow-usda-40",]$meanHeatRatio + (1-as.numeric(usda_40_offset))

sapflow_usda_2022_adjusted[sapflow_usda_2022_adjusted$nodeId == "sapflow-usda-43",]$mr =  sapflow_usda_2022_adjusted[sapflow_usda_hr_may_june_2022$nodeId == "sapflow-usda-43",]$meanHeatRatio + (1-as.numeric(usda_43_offset))
...
sapflow_usda_hr_may_june_2022 %>% filter(1/meanHeatRatio <2) %>%
...
sapflow_usda_2022_adjusted %>% filter(nodeId  %in% c("sapflow-usda-40")) %>%
  mutate(node = as.factor(nodeId),hr = hour(dateP)) %>%
  ggplot() + geom_point(aes(hr,1/mr,color=node) )  +
  facet_grid(~nodeId) +
  theme_classic() + labs(x="Date",y="Mean HR", title = "Node 7-10 USDA - Gravimetric Maize", caption="Source : dotmotelabs.com") +
...
#        filter(date(dateP) == "2020-12-01" ) %>%
 # filter(nodeId=="sapflow-dotmote-labs-2" )  %>% 
  ggplot() +
   geom_point(aes(x = dateP, y=1/sv,color=nodeId)) +
   geom_line(aes(x = dateP, y=1/sv,color=nodeId)) +
...
sapflow_usda_hr_may_june_2022 %>% mutate(yr = year(dateP),mth = month(dateP),dy  = day(dateP),mr=meanHeatRatio) %>%
  filter(yr %in% c(2022) & mth %in% c(05) & dy %in% c(28)) %>%
  mutate (sv = log(1/mr) *3600 * (0.0025/0.6) ,daten = date(dateP))%>% 
#        filter(date(dateP) == "2020-12-01" ) %>%
 # filter(nodeId=="sapflow-dotmote-labs-2" )  %>% 
  ggplot() +
