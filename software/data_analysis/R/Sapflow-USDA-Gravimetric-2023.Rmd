---
title: "Sapflow - USDA Fieldtrial - August 2023"
...
## 50 - Bowen F2 - Full irrigated
## 51 - Bowen F2 - Full irrigated
## 52 - Bowen F2 - Full irrigated

## 57 - Bowen L1 - Limited irrigation (70%)
## 58 - Bowen L1 - Limited irrigation (70%)
## 59 - Bowen L1 - Limited irrigation (70%)
...
```{r pressure, echo=FALSE}
 sapflow_usda_2023_datalog_51%>%
  ...
  ggplot() + 
  geom_smooth(aes(hr,temp2) ,color="blue")  +
    geom_smooth(aes(hr,temp1) ,color="red")  +
  #facet_grid(~node) +
...
```{r , echo=TRUE}
field_repro_all_aug_2023 <- rbind ( sapflow_usda_2023_hr_50_repro, sapflow_usda_2023_hr_51_repro, sapflow_usda_2023_hr_52_repro,
                                    sapflow_usda_2023_hr_57_repro,sapflow_usda_2023_hr_58_repro,sapflow_usda_2023_hr_59_repro)

field_repro_all_aug_2023$datePbythree <- field_repro_all_aug_2023$dateP
# Advance by 3 hours 60*1*60*2
field_repro_all_aug_2023[field_repro_all_aug_2023$nodeID=='sapflow-usda-50',]$datePbythree <- (field_repro_all_aug_2023[field_repro_all_aug_2023$nodeID=='sapflow-usda-50',]$dateP + 10800)

# Advance by 6 hours 60*1*60*2
field_repro_all_aug_2023[field_repro_all_aug_2023$nodeID=='sapflow-usda-51',]$datePbythree <- (field_repro_all_aug_2023[field_repro_all_aug_2023$nodeID=='sapflow-usda-51',]$dateP + 20800)

# Advance by 9 hours 60*1*60*2
field_repro_all_aug_2023[field_repro_all_aug_2023$nodeID=='sapflow-usda-57',]$datePbythree <- (field_repro_all_aug_2023[field_repro_all_aug_2023$nodeID=='sapflow-usda-57',]$dateP + 35800)
...
field_repro_all_aug_2023 %>% 
  mutate(node = as.factor(nodeID),hr = hour(datePbythree),dy = day(datePbythree)) %>%
  mutate(hrp = hour(dateP)) %>%
  mutate(mr = meanHeatRatio) %>%
  filter(node == 'sapflow-usda-50') %>%
  #filter(dy %in% c(14)) %>%
    filter(dy %in% c(14,15,16,17,18)) %>%
  ggplot() + 
  geom_point(aes(hr,1/mr,color=node) )  +
  #geom_point(aes(hrp,1/mr,color='blue') )  +
 facet_grid(~dy) +
  theme_classic() + labs(x="Date",y="Mean HR", title = "Node 50 - Gravimetric Maize", caption="Source : dotmotelabs.com") +
...
field_repro_all_aug_2023 %>% 
  ...
  mutate(hrp = hour(dateP)) %>%
  mutate(mr = meanHeatRatio) %>%
  filter(node %in% c('sapflow-usda-50','sapflow-usda-51','sapflow-usda-57')) %>%
 #filter(dy %in% c(18)) %>%
   filter(dy %in% c(14,15,16,17)) %>%
  ggplot() + 
  geom_smooth(aes(hr,1/mr,color=node) )  +
  #geom_point(aes(hrp,1/mr,color='blue') )  +
 facet_grid(~dy) +
  theme_classic() + labs(x="Date",y="Mean HR", title = "Node 50 - Gravimetric Maize", caption="Source : dotmotelabs.com") +
  scale_x_continuous(breaks = c(0:24)) +
...
sapflow_usda_2023_psy1limited<-  read_csv('./data/usda/psy1/PSY11AOF-limited.csv')
sapflow_usda_2023_psy1limited %>% 
  mutate(datep = as_datetime(`Date and Time`,format="%m/%d/%y %H:%M")) %>% 
    mutate(wp = `Corrected Water Potential (MPa)`) %>% 
  mutate(dy = day(datep), mth=month(datep),hr=hour(datep)) %>%
  filter(dy %in% c(21:24)) %>%
  ggplot() +
   geom_point(aes(datep,wp)) +
     facet_wrap(vars(dy))

wpthreedayslimited <- sapflow_usda_2023_psy1limited %>% 
  mutate(datep = as_datetime(`Date and Time`,format="%m/%d/%y %H:%M")) %>% 
    mutate(wp = `Corrected Water Potential (MPa)`) %>% 
  mutate(dy = day(datep), mth=month(datep),hr=hour(datep)) %>%
  filter(dy %in% c(21:23)) %>%
  dplyr::group_by(mth,dy,hr) %>%
  dplyr::summarise(mwp = mean(wp)) %>%
  as.data.frame()
```
## Med full
```{r , echo=TRUE}

sapflow_usda_2023_psy1medfull<-  read_csv('./data/usda/psy1/LIRF-Psy3-medfull.csv')
sapflow_usda_2023_psy1medfull %>% 
  mutate(datep = as_datetime(`Date and Time`,format="%m/%d/%y %H:%M")) %>% 
    mutate(wp = `Corrected Water Potential (MPa)`) %>% 
    mutate(dy = day(datep), mth=month(datep),hr=hour(datep)) %>%
  filter(dy %in% c(22:23)) %>%
  ggplot() +
   geom_point(aes(datep,wp)) +
     facet_wrap(vars(dy))

wpthreedaysmedfull <- sapflow_usda_2023_psy1medfull %>% 
  mutate(datep = as_datetime(`Date and Time`,format="%m/%d/%y %H:%M")) %>% 
    mutate(wp = `Corrected Water Potential (MPa)`) %>% 
  mutate(dy = day(datep), mth=month(datep),hr=hour(datep)) %>%
  filter(dy %in% c(21:23)) %>%
...
sapflow_usda_2023_psy1basefull<-  read_csv('./data/usda/psy1/LIRF-Psy2-basefull.csv')
sapflow_usda_2023_psy1basefull %>% 
  mutate(datep = as_datetime(`Date and Time`,format="%m/%d/%y %H:%M")) %>% 
    mutate(wp = `Corrected Water Potential (MPa)`) %>% 
    mutate(dy = day(datep), mth=month(datep),hr=hour(datep)) %>%
  filter(dy %in% c(22:23)) %>%
...
wpthreedaysbasefull <- sapflow_usda_2023_psy1basefull %>% 
  mutate(datep = as_datetime(`Date and Time`,format="%m/%d/%y %H:%M")) %>% 
    mutate(wp = `Corrected Water Potential (MPa)`) %>% 
  mutate(dy = day(datep), mth=month(datep),hr=hour(datep)) %>%
  filter(dy %in% c(21:23)) %>%
...
sapflow_usda_2023_waterpotentials<-  read_csv('./data/usda/psy1/LIRF-waterpotentials.csv')
sapflow_usda_2023_waterpotentials %>% 
  mutate(datep = as_datetime(`Date and Time`,format="%m/%d/%y %H:%M")) %>% 
    mutate(wp = `Corrected Water Potential (MPa)`) %>% 
  mutate(dy = day(datep),hr = hour(datep),mth=month(datep) )%>%
  filter(dy %in% c(22:23)) %>%
  ggplot() +
   geom_point(aes(datep,wp)) +
     facet_wrap(vars(dy))
...
sapflow_usda_2023_5minweather<-  read_csv('./data/usda/weather/CoAgMet_2023_LIRF_Data_w_VPD_July_15_to_Sep12_5min (1).csv')
sapflow_usda_2023_5minweatherbst <- sapflow_usda_2023_5minweather %>% 
  mutate(dy = day(TIMESTAMP_MST),hr = hour(TIMESTAMP_MST),mth = month(TIMESTAMP_MST)) %>% as.data.frame()

sapflow_usda_2023_hourlyweather <- sapflow_usda_2023_5minweatherbst %>% dplyr::group_by(mth,dy,hr) %>%
  dplyr::summarise(hrlyvpd = mean(VPD), 
                   hrlytemp = mean(AirTemp_Avg_C),
                   hrlyrh = mean(RH_Avg_fraction),
                   hrlysr = mean(SolRad_W_m2),
                   hrlyrain = mean(Rain_Tot_mm)) %>%
  as.data.frame()
```

# vpd
```{r , echo=TRUE}
sapflow_usda_2023_hourlyweather %>% 
 filter(mth %in% c(8)) %>%
  filter(dy %in% c(21:25)) %>%
  ggplot() +
   geom_point(aes(hr,hrlyvpd)) +
    # facet_wrap(vars(dy))
  facet_grid(~dy) 
```
...
sapflow_usda_2023_5minweather %>% 
  mutate(dy = day(TIMESTAMP_MST),hr = hour(TIMESTAMP_MST),mth = month(TIMESTAMP_MST)) %>% 
 filter(mth %in% c(8)) %>%
  filter(dy %in% c(21:25)) %>%
  ggplot() +
   geom_point(aes(TIMESTAMP_MST,VPD)) +
    # facet_wrap(vars(dy))
  facet_grid(~dy) 
```

## correct for zero flow
filter(node %in% c('sapflow-usda-50','sapflow-usda-51','sapflow-usda-57')) %>%

```{r , echo=TRUE}
field_repro_all_aug_2023$nodeId <- field_repro_all_aug_2023$nodeID 
field_repro_all_aug_2023subset <- field_repro_all_aug_2023 %>%  mutate(node = as.factor(nodeID),hr = hour(datePbythree),dy = day(datePbythree)) %>% filter(dy %in% c(14,15,16,17,18)) %>% 
 # filter(node %in% c('sapflow-usda-50','sapflow-usda-51','sapflow-usda-57')) %>%
  as.data.frame()



usda_50_offset <- field_repro_all_aug_2023subset %>%  mutate(meanhr = meanHeatRatio) %>%
  filter(!meanhr =='NaN') %>% 
  filter(nodeId %in% c("sapflow-usda-50"))  %>% 
  filter( hr %in% c(0,1,2)) %>% summarise(mhr = mean(meanhr))

usda_51_offset <- field_repro_all_aug_2023subset %>%  mutate(meanhr = meanHeatRatio) %>%
  filter(!meanhr =='NaN') %>% 
  filter(nodeId %in% c("sapflow-usda-51"))  %>% 
  filter( hr %in% c(0,1,2)) %>% summarise(mhr = mean(meanhr))

usda_57_offset <- field_repro_all_aug_2023subset %>%  mutate(meanhr = meanHeatRatio) %>%
  filter(!meanhr =='NaN') %>% 
  filter(nodeId %in% c("sapflow-usda-57"))  %>% 
  filter( hr %in% c(0,1,2)) %>% summarise(mhr = mean(meanhr))
```
## Offset for zero flow
```{r , echo=TRUE}
field_repro_all_aug_2023subset_adjusted <- field_repro_all_aug_2023subset
field_repro_all_aug_2023subset_adjusted$mr <- 0
field_repro_all_aug_2023subset_adjusted$type <- ''

field_repro_all_aug_2023subset_adjusted[field_repro_all_aug_2023subset_adjusted$nodeId == "sapflow-usda-50",]$mr =  field_repro_all_aug_2023subset[field_repro_all_aug_2023subset$nodeId == "sapflow-usda-50",]$meanHeatRatio + (1-as.numeric(usda_50_offset))

field_repro_all_aug_2023subset_adjusted[field_repro_all_aug_2023subset_adjusted$nodeId == "sapflow-usda-50",]$type = 'full'
  
field_repro_all_aug_2023subset_adjusted[field_repro_all_aug_2023subset_adjusted$nodeId == "sapflow-usda-51",]$mr =  field_repro_all_aug_2023subset_adjusted[field_repro_all_aug_2023subset$nodeId == "sapflow-usda-51",]$meanHeatRatio + (1-as.numeric(usda_51_offset))

field_repro_all_aug_2023subset_adjusted[field_repro_all_aug_2023subset_adjusted$nodeId == "sapflow-usda-51",]$type = 'full' 
  
field_repro_all_aug_2023subset_adjusted[field_repro_all_aug_2023subset_adjusted$nodeId == "sapflow-usda-57",]$mr =  field_repro_all_aug_2023subset_adjusted[field_repro_all_aug_2023subset$nodeId == "sapflow-usda-57",]$meanHeatRatio + (1-as.numeric(usda_57_offset))

field_repro_all_aug_2023subset_adjusted[field_repro_all_aug_2023subset_adjusted$nodeId == "sapflow-usda-57",]$type = 'limited' 
  
#SE 
field_repro_all_aug_2023subset_adjusted_se <- summarySE(field_repro_all_aug_2023subset_adjusted, measurevar="mr", groupvars=c("nodeId","hr"))

field_repro_all_aug_2023subset_adjusted$dt <- date(field_repro_all_aug_2023subset_adjusted$datePbythree)
#SE by type, dy,hr
field_repro_all_aug_2023subset_adjusted_type_dy_hr_se <- summarySE(field_repro_all_aug_2023subset_adjusted, measurevar="mr", groupvars=c("type","dt","hr"))
...
field_repro_all_aug_2023subset_adjusted %>% mutate(yr = year(datePbythree),mth = month(datePbythree),dy  = day(datePbythree)) %>%
   filter(node %in% c('sapflow-usda-50','sapflow-usda-51','sapflow-usda-57')) %>%
 # filter(yr %in% c(2022) & mth %in% c(05) & dy %in% c(24)) %>%
  mutate (sv = log(1/mr) *3600 * (0.0025/0.6) ,daten = date(datePbythree))%>% 
       filter(dy == 15 ) %>%
 # filter(nodeId=="sapflow-dotmote-labs-2" )  %>% 
  ggplot() +
   geom_point(aes(x = datePbythree, y=sv,color=nodeId)) +
   geom_line(aes(x = datePbythree, y=sv,color=nodeId)) +
    theme_minimal() + scale_x_datetime(date_minor_breaks = "1 day") + 
...
field_repro_all_aug_2023subset_adjusted_fourdays <- field_repro_all_aug_2023subset_adjusted %>% mutate(yr = year(datePbythree),mth = month(datePbythree),dy  = day(datePbythree)) %>%
   filter(node %in% c('sapflow-usda-50','sapflow-usda-51','sapflow-usda-57')) %>%
  filter( dy %in% c(14:17)) %>%
  mutate (sv = log(1/mr) *3600 * (0.0025/0.6) ,daten = date(datePbythree))%>% 
  as.data.frame()
...
field_repro_all_aug_2023subset_adjusted_fourdays %>% 
       filter(dy %in% c(14) ) %>%
 # filter(nodeId=="sapflow-dotmote-labs-2" )  %>% 
  ggplot() +
   geom_point(aes(x = datePbythree, y=sv,color=nodeId)) +
   geom_smooth(aes(x = datePbythree, y=sv,color=nodeId)) +
    theme_minimal() + scale_x_datetime(date_minor_breaks = "1 day") + 
...
# water poyential and vh

```{r , echo=TRUE}
field_repro_all_aug_2023subset_adjusted_threedays <- field_repro_all_aug_2023subset_adjusted_fourdays %>%
  filter(dy %in% c(14:16)) %>% as.data.frame()

#SE
field_repro_all_aug_2023subset_adjusted_type_dy_hr_se3nodes <- field_repro_all_aug_2023subset_adjusted_type_dy_hr_se %>% filter(type != '') %>% 
   filter(dy %in% c(14:16)) %>%
  as.data.frame()

#SE
field_repro_all_aug_2023subset_adjusted_type_dy_hr_se3nodes <- field_repro_all_aug_2023subset_adjusted_type_dy_hr_se %>% filter(type != '') %>% as.data.frame()
...
full <- field_repro_all_aug_2023subset_adjusted_type_dy_hr_se3nodes %>% filter(type =='full') %>% as.data.frame()

limited <- field_repro_all_aug_2023subset_adjusted_type_dy_hr_se3nodes %>% filter(type =='limited') %>% as.data.frame()

fitfield<- lm(full$mr~limited$mr)

comparefullandlimited<-  ggplot()  +geom_point(aes(full$mr,limited$mr)) +
  stat_smooth(aes(full$mr,limited$mr),method = "lm", col = "red",se = FALSE) +
  # geom_smooth(method='lm', formula= full$mr~limited$mr) +
    theme_minimal() +
   labs(title="", y = "Vh - Limited Irrigation",x="Vh - Fully Irrigated",
                colour = "Parameter") + theme(legend.position = c(.2, .8))+
guides(fill = guide_legend(keywidth = 1, keyheight = 1),
    linetype=guide_legend(keywidth = 3, keyheight = 1)) +

annotate("text",x=0.9,y= 0.7, label=paste("Adj R^2 = ",signif(summary(fitfield)$adj.r.squared, 2),
                       " P =",signif(summary(fitfield)$coef[2,4], 2)))

comparefullandlimited
```                       
  

```{r , echo=TRUE}

field_repro_all_aug_2023subset_adjusted_type_dy_hr_se$fulldt <- as.POSIXct(paste(field_repro_all_aug_2023subset_adjusted_type_dy_hr_se$dt, paste0(field_repro_all_aug_2023subset_adjusted_type_dy_hr_se$hr,':',0,":",0), format="%Y-%m-%d %H:%M:%S"))
```  

```{r , echo=TRUE}
allrawfullandlimitedts<-  field_repro_all_aug_2023subset_adjusted_type_dy_hr_se %>%
   filter(type != '') %>%
   ggplot()  +geom_point(aes(fulldt,1/mr,color=type)) +
 # stat_smooth(aes(fulldt,1/mr,group=type,color=type),method = "loess",se = FALSE) +
  # geom_smooth(method='lm', formula= full$mr~limited$mr) +
    theme_minimal() +
   labs(title="", y = "Vh - Limited Irrigation",x="Vh - Fully Irrigated",
                colour = "Parameter") + theme(legend.position = c(.2, .8))+
...
allrawfullandlimitedts
``` 
# three day plot 

```{r , echo=TRUE}
allrawfullandlimited<-  field_repro_all_aug_2023subset_adjusted_type_dy_hr_se %>%
   mutate(dy = day(dt)) %>%
   filter(type != '') %>%
  filter(dy %in% c(14,15,16)) %>%
   ggplot()  +geom_point(aes(hr,1/mr,color=type)) +
  stat_smooth(aes(hr,1/mr,group=type,color=type),method = "loess",se = FALSE) +
  # geom_smooth(method='lm', formula= full$mr~limited$mr) +
    theme_minimal() +
   labs(title="",x="Time (Hour)",
                colour = "Treatment") + theme(legend.position = c(.1, .8))+
guides(fill = guide_legend(keywidth = 1, keyheight = 1),
    linetype=guide_legend(keywidth = 3, keyheight = 1)) +
  facet_wrap(.~dt) +
      scale_y_continuous(
...
#+
#annotate("text",x=2,y= 0.7, label=paste("Adj R^2 = ",signif(summary(fitfield)$adj.r.squared, 2),
#                       " P =",signif(summary(fitfield)$coef[2,4], 2)))

allrawfullandlimited
```  
```{r , echo=TRUE}
threedaysdata<-  field_repro_all_aug_2023subset_adjusted_type_dy_hr_se %>%
   mutate(dy = day(dt)) %>%
   filter(type != '') %>%
  filter(dy %in% c(14,15,16)) %>% as.data.frame()

```  

#Water poyential and Vh (full)

```{r , echo=TRUE}
fulldatavh <- threedaysdata %>% filter(type=='full') %>% select(dy,hr,mr,se) %>% as.data.frame()
fulldatavh[fulldatavh$dy==14,]$dy <- 21
fulldatavh[fulldatavh$dy==15,]$dy <- 22
fulldatavh[fulldatavh$dy==16,]$dy <- 23

fullvhwp <- left_join(fulldatavh,wpthreedaysbasefull , by = c('dy','hr'))

lmfullvhwpfit <- lm(fullvhwp$mr ~ fullvhwp$mwp)
fieldfullexp_wpandvh <- fullvhwp%>% ggplot(aes(mwp,mr))  +geom_point() +
  stat_smooth(method = "lm", col = "red",se = FALSE) +
    theme_minimal() +
   labs(title="Fully Irrigated", x = "Water Potential (MPa)",
                colour = "Parameter") + theme(legend.position = c(.2, .8))+
guides(fill = guide_legend(keywidth = 1, keyheight = 1),
    linetype=guide_legend(keywidth = 3, keyheight = 1)) +
      scale_y_continuous(
...
annotate("text",x=-0.2,y= 0.7, label=paste("R^2 = ",signif(summary(lmfullvhwpfit)$r.squared, 2),
                       " P =",signif(summary(lmfullvhwpfit)$coef[2,4], 2)))
fieldfullexp_wpandvh
```
#Water poyential and Vh (full)

```{r , echo=TRUE}
limiteddatavh <- threedaysdata %>% filter(type=='limited') %>% select(dy,hr,mr,se) %>% as.data.frame()
limiteddatavh[limiteddatavh$dy==14,]$dy <- 21
limiteddatavh[limiteddatavh$dy==15,]$dy <- 22
limiteddatavh[limiteddatavh$dy==16,]$dy <- 23

limitedvhwp <- left_join(limiteddatavh,wpthreedayslimited , by = c('dy','hr'))

lmlimitedvhwpfit <- lm(limitedvhwp$mr ~ limitedvhwp$mwp)
fieldlimitedexp_wpandvh <- limitedvhwp%>% ggplot(aes(mwp,mr))  +geom_point() +
  stat_smooth(method = "lm", col = "red",se = FALSE) +
    theme_minimal() +
   labs(title="Limited Irrigation", x = "Water Potential (MPa)",
                colour = "Parameter") + 
    scale_y_continuous(
    # Features of the first axis
    name = expression(~V[h] (cm~h^{"-1"}))
...
annotate("text",x=-1,y= 0.7, label=paste("R^2 = ",signif(summary(lmlimitedvhwpfit)$r.squared, 2),
                       " P =",signif(summary(lmlimitedvhwpfit)$coef[2,4], 2))  )
fieldlimitedexp_wpandvh
```
...
hrlyweatherthreedays <- sapflow_usda_2023_hourlyweather %>% filter(dy %in% c(21,22,23)) %>%
   filter(mth %in% c(8)) %>%
    as.data.frame()
fullvhweather <- left_join(fulldatavh,hrlyweatherthreedays , by = c('dy','hr'))

fullvhweather$sv <- log(1/fullvhweather$mr) *3600 * (0.0025/0.6) 
fullvhweatherfit <- lm(fullvhweather$sv ~ fullvhweather$hrlyvpd)
 
fieldfullexp_vpdandvh <- fullvhweather%>% 
  ggplot(aes(hrlyvpd,sv))  +geom_point() +
  stat_smooth(method = "lm", col = "red",se = FALSE) +
    theme_minimal() +
   labs(title="Fully Irrigated", x = "VPD",
                colour = "Parameter") + theme(legend.position = c(.2, .8))+
...
annotate("text",x=1,y= 5, label=paste("R^2 = ",signif(summary(fullvhweatherfit)$adj.r.squared, 2),
                       " P =",signif(summary(fullvhweatherfit)$coef[2,4], 2)))
fieldfullexp_vpdandvh
```
...
hrlyweatherthreedays <- sapflow_usda_2023_hourlyweather %>% filter(dy %in% c(21,22,23)) %>%
   filter(mth %in% c(8)) %>%
    as.data.frame()
limitedvhweather <- left_join(limiteddatavh,hrlyweatherthreedays , by = c('dy','hr'))

limitedvhweather$sv <- log(1/limitedvhweather$mr) *3600 * (0.0025/0.6) 
limitedvhweatherfit <- lm(limitedvhweather$sv ~ limitedvhweather$hrlyvpd)
 
fieldlimitedexp_vpdandvh <- limitedvhweather%>% 
  ggplot(aes(hrlyvpd,sv))  +geom_point() +
  stat_smooth(method = "lm", col = "red",se = FALSE) +
    theme_minimal() +
   labs(title="Limited Irrigation", x = "VPD",
                colour = "Parameter") + theme(legend.position = c(.2, .8))+
...
annotate("text",x=1,y= 5, label=paste("R^2 = ",signif(summary(limitedvhweatherfit)$adj.r.squared, 2),
                       " P =",signif(summary(limitedvhweatherfit)$coef[2,4], 2)))
fieldlimitedexp_vpdandvh
```


```{r , echo=FALSE}
library(cowplot)
plot_grid(fieldlimitedexp_vpdandvh,fieldfullexp_vpdandvh, 
          fieldlimitedexp_wpandvh,fieldfullexp_wpandvh, 
  nrow = 2,
  ncol = 2,
  labels = "AUTO",
  label_size = 12,
  ...
  axis="tb",
  scale = c(1, 1))
ggsave("figs/fig3_confsap.pdf", width = 20, height = 20, units = "cm")
ggsave("figs/fig3_confsap.png", width = 20, height = 20, units = "cm")
