...
fieldfullexp_wpandvh <- fullvhwp%>% ggplot(aes(mwp,mr))  +geom_point() +
  stat_smooth(method = "lm", col = "red",se = FALSE) +
    theme_minimal() +
   labs(title="Fully Irrigated", x = "Water Potential (MPa)",y="Vh",
                colour = "Parameter") + theme(legend.position = c(.2, .8))+
guides(fill = guide_legend(keywidth = 1, keyheight = 1),
    linetype=guide_legend(keywidth = 3, keyheight = 1)) +

annotate("text",x=-0.1,y= 0.7, label=paste("Adj R^2 = ",signif(summary(lmfullvhwpfit)$adj.r.squared, 2),
...
## Vh and vpd   (full)
```{r , echo=TRUE}
fieldexp_vpdandvh <- df_plot_douglasii%>% ggplot(aes(vh,E))  +geom_point() +
  stat_smooth(method = "lm", col = "red",se = FALSE) +
    theme_minimal() +
   labs(title="Douglasii", y = "Transpiration [E]",x="Vh",
                colour = "Parameter") + theme(legend.position = c(.2, .8))+
guides(fill = guide_legend(keywidth = 1, keyheight = 1),
    linetype=guide_legend(keywidth = 3, keyheight = 1)) +

annotate("text",x=0,y= 15, label=paste("Adj R^2 = ",signif(summary(fit_douglasii)$adj.r.squared, 5),
                       " P =",signif(summary(fit_douglasii)$coef[2,4], 5)))

```


```{r , echo=FALSE}
plot_grid(kelloggii_eandhr,douglasii_eandhr,
  nrow = 2,
  ncol = 2,
  labels = "AUTO",
  label_size = 12,
